...
def l_encounter_pokemon_request(api, account, encounter_id, fort_id, scan_location):
...
def update_inventory_totals(inventory):
    ...
    for item_id in inventory:
        if item_id in ['total', 'balls', 'totalDisks']:
            continue
        if item_id in ball_ids:
            balls += inventory[item_id]
        if item_id in lure_ids:
            totalDisks += inventory[item_id]
        total_items += inventory[item_id]
    inventory['totalDisks'] = totalDisks    # inventory.get(ITEM_TROY_DISK, 0)
